...
namespace Cassam.Core.Persistence.Migrations
{
    ...
    public partial class DocumentoElectronicoEstadoCheck : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // gates INSERT).
            //
            // Case handling: EF Core's HasConversion<string>() stores
            // the C# enum name as PascalCase ("Draft", "Signed"),
            // while the spec and this trigger function use UPPER-CASE
            // labels. The CHECK and trigger both wrap their inputs in
            // UPPER(...) so the application layer's PascalCase writes
            // pass the constraint and trigger identically.

            // 1. CHECK constraint.
            migrationBuilder.Sql(@"
                ALTER TABLE documentos_electronicos
                  ADD CONSTRAINT ck_documentos_estado_valid
                  CHECK (UPPER(estado) IN (
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    valid_transition := CASE
                        WHEN UPPER(OLD.estado) = 'DRAFT' AND UPPER(NEW.estado) = 'SIGNED' THEN TRUE
                        WHEN UPPER(OLD.estado) = 'SIGNED' AND UPPER(NEW.estado) IN ('QUEUED', 'TRANSMITTED', 'REQUIRES_REHABILITACION') THEN TRUE
                        WHEN UPPER(OLD.estado) = 'QUEUED' AND UPPER(NEW.estado) IN ('TRANSMITTED', 'REQUIRES_REHABILITACION') THEN TRUE
                        WHEN UPPER(OLD.estado) = 'TRANSMITTED' AND UPPER(NEW.estado) IN ('ACCEPTED', 'REJECTED', 'PENDING', 'VOIDED', 'REQUIRES_REHABILITACION') THEN TRUE
                        WHEN UPPER(OLD.estado) = 'PENDING' AND UPPER(NEW.estado) IN ('ACCEPTED', 'REJECTED', 'VOIDED', 'REQUIRES_REHABILITACION') THEN TRUE
                        WHEN UPPER(OLD.estado) = 'REQUIRES_REHABILITACION' AND UPPER(NEW.estado) = 'SIGNED' THEN TRUE
                        ELSE FALSE
                    END;
